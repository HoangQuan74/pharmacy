...
const getFreeMembers = async (req: Request, res: Response) => {
  const userId = req.userData.id;
  const projectId = parseInt(req.params.id);
  const projectsv = new ProjectService();
  const isMember = await projectsv.isMember(projectId, userId);
  ...
  const freeMembers = members.filter(member => !taskMemberIds.includes(member.id))

  return res.status(200).json(freeMembers);
};
